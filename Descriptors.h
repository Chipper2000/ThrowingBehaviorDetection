...
//余弦距离表示两个向量之间方向的相似性，取值在0-2之间，取值越小代表方向越接近
double cosine_distance_flag(Point v1,Point v2,int& flag)
{
    double dot = 0.0, denom_a = 0.0, denom_b = 0.0,cosine_distance=0.0 ;
    
        dot = (v1.x)*(v2.x)+(v1.y)*(v2.y) ;
        denom_a =(v1.x)*(v1.x)+(v1.y)*(v1.y) ;
        denom_b =(v2.x)*(v2.x)+(v2.y)*(v2.y) ;
		 if(denom_a!=0 && denom_b!=0)
            cosine_distance=1-(dot / (sqrt(denom_a) * sqrt(denom_b)));
		else
			flag-=1;
...
double cosine_distance(Point v1,Point v2)
{
    double dot = 0.0, denom_a = 0.0, denom_b = 0.0,cosine_distance=0.0 ;
...
//calcuate  direction features
void orientation(std::vector<Point2f>& track, double& mean_cos_distance, double& var_cos_distance, double& max_cos_distance,double& min_cos_distance)
{
	std::vector<Point2f> temp_track(16);
	int cos_size = track.size()-2;
	int flag_size=track.size()-2;
	for(int i = 0; i < cos_size; i++) {
		Point v1=track[i+1]-track[i];
		Point v2=track[i+2]-track[i+1];
		mean_cos_distance += cosine_distance_flag(v1,v2,flag_size);
	}
	float cos_norm = 1./flag_size;
	mean_cos_distance *= cos_norm;

	for(int i = 0; i < cos_size; i++) {
		Point v1=track[i+1]-track[i];
		Point v2=track[i+2]-track[i+1];
		double temp = cosine_distance(v1,v2) - mean_cos_distance;
		var_cos_distance += temp*temp;
	}
	var_cos_distance *= cos_norm;
	var_cos_distance = sqrt(var_cos_distance);
	...
	for(int i = 0; i < cos_size; i++) {
		Point v1=track[i+1]-track[i];
		Point v2=track[i+2]-track[i+1];
		float temp_cos_distance =  cosine_distance(v1,v2);

		if(temp_cos_distance > temp_max)
			temp_max =temp_cos_distance;

		if(temp_cos_distance < temp_min)
			temp_min =temp_cos_distance;	
	}
	max_cos_distance=temp_max;
	min_cos_distance=temp_min;
}

// check whether a trajectory is valid or not
bool IsValid(std::vector<Point2f>& track, float& mean_x, float& mean_y, float& var_x, float& var_y, float& length,double& mean_cos_distance, double& var_cos_distance, double& max_cos_distance,double& min_cos_distance)
{
	std::vector<Point2f> temp_track(16);
	int size = track.size();
	float norm = 1./size;
	...
	for(int i = 0; i < size-1; i++) {
		temp_track[i] = track[i+1] - track[i];
		float temp = sqrt(temp_track[i].x*temp_track[i].x + temp_track[i].y*temp_track[i].y);
...
bool alwaysValid(std::vector<Point2f>& track, float& mean_x, float& mean_y, float& var_x, float& var_y, float& length)
{
	std::vector<Point2f> temp_track(16);
	int size = track.size();
	float norm = 1./size;
...
void DrawTrack(const std::vector<Point2f>& point, const int index, const float scale, Mat& image)
{
  ...
  //printf("0\t");
	circle(image, point0, 3, Scalar(0,0,255), -1, 8, 0);
}

void DrawThrowTrack(const std::vector<Point2f>& point, const int index, const float scale, Mat& image)
{
	Point2f point0 = point[0];
	point0 *= scale;
	...
	for (int j = 1; j <= index; j++) {
		...
		circle(image, point0, 2, Scalar(0,255,0), -1, 8, 0);
		line(image, point0, point1, Scalar(0,0,255), 3, 8, 0);
		point0 = point1;
    //printf("(%f,%f) \t \n",point0.x, point0.y);
	}
  //printf("------------------------------\n");
	//circle(image, point0, 3, Scalar(0,255,0), -1, 8, 0);
}

void DrawCircle(const std::vector<Point2f>& point, const int index, const float scale, Mat& image)
{
	Point2f point0 = point[0];
	point0 *= scale;
